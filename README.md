...
- 원본 저장소를 fork 하게 해서 fork한 저장소에서
- commit 을 하여 원본 소유자에게 contribute -> pull request를 통해
...
- 충돌이 있다면, rebase 도중 충돌파일을 수정하고, rebase를 계속한다.

- (소스트리) 액션 > 재배치 계속

- 원격 저장소에 push를 할떄는 커밋 이력이 조작되었기 때문에 강제 푸시를 해야 한다.
